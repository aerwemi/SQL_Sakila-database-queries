...
-- change frist name when both condition agree `GROUCHO WILLIAMS`
UPDATE actor
...
-- check if the change took place which prodcue only one record 
SELECT first_name, last_name
FROM actor
WHERE 
	last_name = 'WILLIAMS'
...
-- change the first name of that one record back to GROUCHO
...
-- change the first name of that one record back to GROUCHO - as long as there is one recrod - one cond is ok where first name is HARPO 
...
-- check if HARPO is there - which none 
SELECT first_name, last_name
FROM actor
WHERE 
    first_name = 'HARPO'; 
...
-- check address for cols to join and common id col 
SELECT *
FROM address; 
...
FROM staff
INNER JOIN address
USING(address_id);  -- ON address_id could be used as well 
